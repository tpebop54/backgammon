...
const BackgammonBoard: React.FC = () => {
  ...
  const renderPoint = (pointIndex: number, isTopRow: boolean) => {
    ...
    const isCurrentPlayerPiece = (gameState.currentPlayer === 'white' && pieces > 0) ||
                                 (gameState.currentPlayer === 'black' && pieces < 0);
    const hasValidMoves = gameState.possibleMoves.some(move => move.from === pointIndex);
    const canDrag = isCurrentPlayerPiece && hasValidMoves;
    ...
    for (let i = 0; i < Math.min(absCount, maxVisible); i++) {
      const isTopPiece = i === Math.min(absCount, maxVisible) - 1;
      pieceElements.push(
        <div
          key={i}
          draggable={canDrag && isTopPiece}
          ...
          className={`w-8 h-8 rounded-full border-2 ${
            ...
            canDrag && isTopPiece ? 'cursor-move hover:scale-110' : 'cursor-pointer'
          }`}
        />
      );
    }
      ...
      pieceElements.push(
        <div
          ...
          className={`w-8 h-8 rounded-full border-2 flex items-center justify-center text-xs font-bold ${
            ...
            canDrag ? 'cursor-move hover:scale-110' : 'cursor-pointer'
          }`}
        >
          +{absCount - maxVisible}
        </div>
  ...
  const renderBar = () => {
    ...
    const canDragFromBar = currentPlayerOnBar && hasValidBarMoves;
    
    return (
      <div 
        className={`w-16 h-40 bg-amber-900 flex flex-col items-center justify-center gap-2 cursor-pointer transition-colors ${
          selectedPoint === -1 ? 'ring-4 ring-blue-400' : ''
        ...
        {blackOnBar && (
          <div className="flex flex-col items-center">
            <div 
              ...
              onDragEnd={handleDragEnd}
              className={`w-8 h-8 rounded-full bg-gray-800 border-2 border-white select-none transition-transform ${
                canDragFromBar && gameState.currentPlayer === 'black' ? 'cursor-move hover:scale-110' : ''
              }`}
            />
        ...
        {whiteOnBar && (
          <div className="flex flex-col items-center">
            <div 
              ...
              onDragEnd={handleDragEnd}
              className={`w-8 h-8 rounded-full bg-white border-2 border-gray-800 select-none transition-transform ${
                canDragFromBar && gameState.currentPlayer === 'white' ? 'cursor-move hover:scale-110' : ''
              }`}
            />
    ...
    <div className="flex flex-col items-center p-8 bg-amber-100 min-h-screen">
          ...
          <div className="flex items-center justify-center gap-4 mt-4">
            ...
            <div className="flex gap-2">
              {gameState.dice.map((die, index) => (
                <div
                  key={index}
                  className={`w-12 h-12 border-2 border-gray-800 rounded-lg flex items-center justify-center text-2xl font-bold shadow-lg transition-all ${
                    gameState.usedDice[index] ? 'bg-gray-400 text-gray-600 scale-90' : 'bg-white text-black'
                  }`}
                >
                  {die}
                </div>
      ...
      <div className="mt-6 p-4 bg-white rounded-lg shadow-lg max-w-2xl">
        ...
        <ul className="text-sm space-y-1">
          <li>• <strong>Drag & Drop:</strong> Drag checkers to valid destinations</li>
          <li>• <strong>Click to Play:</strong> Click pieces to select, then click destinations</li>
          <li>• <strong>Visual Cues:</strong> Green highlights show valid moves</li>
          <li>• <strong>Bar Priority:</strong> Move pieces from the bar first if any are there</li>
          <li>• <strong>Bearing Off:</strong> Drag to home area when all pieces are in your home board</li>
          <li>• <strong>Win Condition:</strong> First to bear off all 15 pieces wins!</li>
        </ul>
      </div>

      {/* Score */}
      <div className="mt-4 flex gap-8 text-lg font-bold">
        <div className="text-gray-800">WHITE: {gameState.home.white}/15</div>
        <div className="text-gray-800">BLACK: {gameState.home.black}/15</div>
      </div>
    </div>
  );
};
